...
int ecc_verify_key(ecc_key *key)
{
  ...
  void *order = NULL;
  void *a = NULL;
  ecc_point *test_out = NULL;
  test_out = XMALLOC(sizeof(ecc_point));

  if (mp_init_multi(&(test_out->x), &(test_out->y), &(test_out->z), &order, &prime, NULL) != CRYPT_OK) {
    return CRYPT_MEM;
  }
  ...
  if((err = ltc_ecc_mulmod(order, &(key->pubkey), test_out, a, prime, 1)) != CRYPT_OK)  { goto error; }

  err = CRYPT_OK;
error:
  mp_clear_multi(prime, order, test_out->z, test_out->y, test_out->x, NULL);
  return err;
}
